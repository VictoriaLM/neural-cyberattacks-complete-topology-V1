...
public:
  ...
  // Definition of the function which allow the execution of cyberattack.
  void set_status_attack( const DictionaryDatum& );
...
private:
  ...
  struct State_
  {
    ...
    // This function allows the execution of the attack.
    void set_attack( const DictionaryDatum&, const Parameters_&, double, Node* );  
...
{
  // In this function, it reads which cyberattack will be executed and when.
  Time const& clock_ = kernel().simulation_manager.get_clock();
  Dictionary type = kernel().simulation_manager.get_type_attack_dict();
  Dictionary FLO = kernel().simulation_manager.get_FLO_dict();
  Dictionary JAM = kernel().simulation_manager.get_JAM_dict();

  if(type["attack"] == "FLO")
  {
    int instant = std::stoi(FLO["instants_attack"]);
    if(clock_.get_ms() == instant)
    {
      set_status_attack(d);
    }
  }else if(type["attack"] == "JAM")
  {
    int init_instant = std::stoi(JAM["init_attack"]);
    int end_instant = std::stoi(JAM["end_attack"]);
...
// It changes flags that allow the execution of cyberattacks.
inline void
glif_psc::set_status_attack( const DictionaryDatum& d )
{

  Parameters_ ptmp = P_;                       // temporary copy in case of errors
  const double delta_EL = ptmp.set( d, this ); // throws if BadProperty
  State_ stmp = S_;                            // temporary copy in case of errors
  stmp.set_attack( d, ptmp, delta_EL, this );  // Function defined in glif_psc.cpp
  ArchivingNode::set_status( d );
