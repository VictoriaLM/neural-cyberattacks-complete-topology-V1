...
// This function changes the flags of the neuron that allow the execution of the cyberattack and its propagation during 
// the simulation.
void
nest::glif_psc::State_::set_attack( const DictionaryDatum& d, const Parameters_& p, double delta_EL, Node* node )
{
  if ( updateValueParam< double >( d, names::V_m, U_, node ) )
  {
    U_ -= p.E_L_;
  ...
  if ( updateValueParam< double >( d, names::threshold_spike, threshold_spike_, node ) and p.has_theta_spike_ )
  {
    throw BadProperty( "Threshold spike component is not supported or settable in the current model mechanisms." );
  }
  if ( updateValueParam< double >( d, names::threshold_voltage, threshold_voltage_, node ) and p.has_theta_voltage_)
  {
    throw BadProperty( "Threshold voltage component is not supported or settable in the current model mechanisms." );
  }
}
