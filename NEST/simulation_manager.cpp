...
/***********************************************************************/
//Dictionaries of attacks
void
nest::SimulationManager::load_dictionary_()
{
  std::ifstream attack_file, FLO_file, JAM_file;

  attack_file.open("/home/victoria/type_attack.txt");
  FLO_file.open("/home/victoria/FLO_attributes.txt");
  JAM_file.open("/home/victoria/JAM_attributes.txt");
 
  if(attack_file.is_open() && FLO_file.is_open() && JAM_file.is_open())
  {
    std::string text_type,text_flo,text_jam,text_pop,text_neuron, text_conn;
    ...
    while(getline(FLO_file, text_flo))
    {
      std::string key,value;

      key = text_flo.substr(0,text_flo.find(':'));
      value = text_flo.substr(text_flo.find(":") + 1);
      FLO_dict[key] = value;
    }

    while(getline(JAM_file, text_jam))
    {
      std::string key,value;

      key = text_jam.substr(0,text_jam.find(':'));
      value = text_jam.substr(text_jam.find(":") + 1);
      JAM_dict[key] = value;
    }
  }
...
//FLO attack
/*
  * A list is created exclusively for when a specific number of neurons are attacked. 
  *
  * This list consists of the IDs of the random neurons that will be targeted.
  * 
  * During the process, the neurons are iterated to obtain their IDs, 
  * aiming to verify if they are on the list. If so, the cyberattack is executed 
  * by changing their membrane voltage to their corresponding threshold voltage.
  * 
  * A second experiment has been created for this cyberattack, which consists of 
  * adding a specific voltage to the membrane voltage. This study was done to study 
  * different voltage levels in the same number of affected neurons.
*/
void
nest::SimulationManager::FLO_attack_()
{
  std::ostringstream os;
  LOG( M_INFO, "SimulationManager::FLO_attack_", os.str() );
  const size_t tid = kernel().vp_manager.get_thread_id();

  std::vector<int> list_random;
  ...
  while (unique_numbers.size() < std::stoi(FLO_dict["n_neurons"])) {
    ...
    if (unique_numbers.find(random_num) == unique_numbers.end()) {
      ...
      list_random.push_back(random_num);
      unique_numbers.insert(random_num);
    }
  }

  if(std::stoi(FLO_dict["n_neurons"]) < 230000)
  {
      ...
      {  
        ...
        {  
          if(std::stoi(FLO_dict["v_increment"]) == 551)
          {
            ...
            float value_VM = dict_consulting["V_m"].operator double() + std::stoi(FLO_dict["v_increment"]);
  ...
  }else if(std::stoi(FLO_dict["n_neurons"]) == 230000)
  {
...
/*
  ...
  * During the process, the neurons are iterated to obtain their IDs, 
  * aiming to verify if they are on the list. If so, the cyberattack is executed 
  * by changing their membrane voltage to their corresponding reset voltage.
*/
void
nest::SimulationManager::JAM_attack_()
{
  std::ostringstream os;
  LOG( M_INFO, "SimulationManager::JAM_attack_", os.str() );
  const size_t tid = kernel().vp_manager.get_thread_id();
  ...
  while (unique_numbers.size() < std::stoi(JAM_dict["n_neurons"])) {
    int random_num = dis(gen);
  ...
  if(std::stoi(JAM_dict["n_neurons"]) < 230000)
  {
    for ( SparseNodeArray::const_iterator i = kernel().node_manager.get_local_nodes( tid ).begin();
      i != kernel().node_manager.get_local_nodes( tid ).end(); ++i )
    {
  ...
  }else if(std::stoi(JAM_dict["n_neurons"]) == 230000)
  {
    for ( SparseNodeArray::const_iterator i = kernel().node_manager.get_local_nodes( tid ).begin();
      i != kernel().node_manager.get_local_nodes( tid ).end(); ++i )
    {
...
#pragma omp master
        {
          ...
          // It is verified which attack will be used and at what instant or interval it will be executed.
          if(type_attack["attack"] == "FLO")
          {
            int instant = std::stoi(FLO_dict["instants_attack"]);
            if(clock_.get_ms() == instant)
            {
              FLO_attack_();
            }
          }else if(type_attack["attack"] == "JAM")
          {
            int init_instant = std::stoi(JAM_dict["init_attack"]);
            int end_instant = std::stoi(JAM_dict["end_attack"]);
